...
function Get-ProxyTarget([string]$proxyServer) {
    ...
    $target = $null
    if ($proxyServer -match '(?:^|;)https=([^;]+)') { $target = $Matches[1] }
    elseif ($proxyServer -match '(?:^|;)http=([^;]+)') { $target = $Matches[1] }
    else { $target = ($proxyServer -split ';')[0] }
    if ([string]::IsNullOrWhiteSpace($target)) { return $null }
    $target = $target.Trim()
    if ($target -notmatch '^[a-zA-Z][a-zA-Z0-9+.-]*://') { $target = 'http://' + $target }
    return $target
}
...
Clear-GitProxy
if ($settings.AutoConfigURL) { Write-Host '[PROXY] PAC proxy detected. Git will use direct connection.' -ForegroundColor Yellow }
else { Write-Host '[PROXY] Windows proxy is disabled. Git will use direct connection.' -ForegroundColor Cyan }
exit 0
