...
<div class="xm-home-grid">
  <aside class="xm-left-menu">
    ...
    {% if item.children %}
      ...
      <button class="xm-filter-btn xm-filter-child" data-filter="{{ item.key }}" data-subfilter="{{ child.key }}">└ {{ child.label }}</button>
      {% endfor %}
    {% endif %}
    {% endfor %}
  </aside>
  ...
  <main class="xm-center">
    ...
    <section class="xm-post-list" id="xm-post-list">
      ...
      <article class="xm-post-card" data-category="{{ post.categories | first }}" data-subcategory="{{ post.subcategory | default: '' }}" data-search="{{ post.title | escape }} {{ post.excerpt | strip_html | strip_newlines | escape }}">
...
<script>
(function(){
  var buttons=[].slice.call(document.querySelectorAll('.xm-filter-btn')),cards=[].slice.call(document.querySelectorAll('.xm-post-card')),allSlides=[].slice.call(document.querySelectorAll('.xm-slide'));
  ...
  var activeFilter='all',activeSub='',visibleSlides=[],current=0,timer=null;
  function rebuildSlides(){visibleSlides=allSlides.filter(function(s){return s.dataset.slideCategory===activeFilter;});allSlides.forEach(function(s){s.classList.remove('active');});if(dots)dots.innerHTML='';current=0;if(!visibleSlides.length)return;visibleSlides.forEach(function(_,i){var b=document.createElement('button');b.type='button';b.onclick=function(e){e.preventDefault();showSlide(i);restart();};dots.appendChild(b);});showSlide(0);restart();}
  function showSlide(i){if(!visibleSlides.length)return;current=(i+visibleSlides.length)%visibleSlides.length;visibleSlides.forEach(function(s,j){s.classList.toggle('active',j===current);});if(dots)[].slice.call(dots.children).forEach(function(d,j){d.classList.toggle('active',j===current);});}
  function restart(){clearInterval(timer);if(visibleSlides.length>1)timer=setInterval(function(){showSlide(current+1);},5000);}
  function applyPosts(){var q=(search&&search.value||'').trim().toLowerCase(),shown=0;cards.forEach(function(card){var cat=card.dataset.category||'',sub=card.dataset.subcategory||'',text=(card.dataset.search||'').toLowerCase();var show=(activeFilter==='all'||cat===activeFilter)&&(!activeSub||sub===activeSub)&&(!q||text.indexOf(q)>-1);card.classList.toggle('hidden',!show);if(show)shown++;});if(empty)empty.classList.toggle('show',shown===0);}
  buttons.forEach(function(btn){btn.onclick=function(){buttons.forEach(function(b){b.classList.remove('active');});btn.classList.add('active');activeFilter=btn.dataset.filter||'all';activeSub=btn.dataset.subfilter||'';applyPosts();rebuildSlides();};});
  if(prev)prev.onclick=function(e){e.preventDefault();showSlide(current-1);restart();};if(next)next.onclick=function(e){e.preventDefault();showSlide(current+1);restart();};if(search)search.oninput=applyPosts;applyPosts();rebuildSlides();
})();
</script>
